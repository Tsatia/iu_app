...
def get_id_and_link(url):
    """Get the id of a given url. e.g. https://swapi.dev/api/vehicles /1/ should return 1"""
...
def add_vehicle_associated_data(table_name, obj_id, obj_id_name, obj_link, obj_link_name, cursor):
    """Add Records to the satellite classes of vehicles """
...
def add_vehicle_data(request_response, vehicle_url, vehicle_pilot_id, vehicle_film_id, cursor):
    """ Add vehicle records into the table vehicles . See file star_wars.sql for more information. """
    statement = f"INSERT INTO vehicles  (vehicle_id, vehicle_pilot_id, vehicle_film_id, name, " \
                f"model, manufacturer, cost_in_credits," \
                " length, max_atmosphering_speed, crew, passengers, cargo_capacity," \
                " consumables, vehicle_class, created, edited, url) " \
                "Values  (%s,  %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
    data = (get_id_and_link(vehicle_url)[0],
            vehicle_pilot_id,
            vehicle_film_id,
            ...
            request_response.get("vehicle_class"),
...
def load_all_data():
    """Load at once data into vehicles , vehicles _film, vehicles _specy, vehicles _vehicle, vehicles _vehicle"""
    try:
        config = {
            'user': 'root',
            'password': 'root',
        ...
        for url in get_obj_urls("vehicles"):
            ...
            for index in range(0, len(films_links)):
                add_vehicle_data(response, url, pilots_links[index][0], films_links[index][0], cursor)
                add_vehicle_associated_data("vehicle_film", films_links[index][0], "vehicle_film_id",
                                            films_links[index][1],
                                            "link",
                                            cursor)
                add_vehicle_associated_data("vehicle_pilot", pilots_links[index][0], "vehicle_pilot_id",
        ...
        cursor.close()
        connection.close()
        print("Successfully added entry to database --> vehicles  Tables")
